        ...
        <div className="p-8 bg-white shadow mt-24">
          <div className="grid grid-cols-1 md:grid-cols-3">
            <div className="grid grid-cols-2 text-center order-last md:order-first mt-20 md:mt-0">
              <div>
                <p className="font-bold text-gray-700 text-xl">{data?.data.quotes.length}</p>
                <p className="text-gray-400">Quote</p>
              </div>
              <div>
                <p className="font-bold text-gray-700 text-xl">{data?.data.favorites.length}</p>
          ...
          <div className="mt-20 text-center border-b pb-12">
            ...
            <p className="mt-8 text-gray-500">
              {
                new Set(data?.data?.quotes?.map(({category}: SingleQuoteProps, index: number) => {
